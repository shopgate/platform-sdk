...
class I18n {
  constructor (locale = 'en') {
    ...
    locales.forEach((locale) => {
      const localeFilePath = path.resolve(rootDirectory, config.get('localesDirectory'), `${locale}.json`)
...
module.exports = (modulePath) => {
  ...
  const moduleNamespace = path.relative(rootDirectory, modulePath).replace(/\.js$/i, '').split(path.sep).join('/')
  return (key, data) => {
