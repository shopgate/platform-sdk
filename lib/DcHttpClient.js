...
// Error code the developer connector uses when the pipeline controller has no /encryptionKeys route.
// Checking it distinguishes an outdated PLC from a 404 caused by an outdated developer connector.
const ENCRYPTION_KEYS_UNSUPPORTED_CODE = 'EPLCNOENCRYPTIONKEYS'

// Error code the developer connector uses when the pipeline controller has no /auth route,
// i.e. it predates context.user support.
const AUTH_UNSUPPORTED_CODE = 'EPLCNOAUTH'
...
class DcHttpClient {
  ...
  /**
   * Fetches the public encryption keys configured on the pipeline controller so the SDK can
   * encrypt payloads locally (instead of sending plaintext over the wire). Public keys are not secret.
   *
   * Throws a NotFoundError if the pipeline controller has no encryption key support at all, so that
   * callers can report an outdated PLC instead of a generic failure.
   *
   * @param {string} appId
   * @returns {Promise<Array<{alias: string, publicKeyPem: string}>>}
   */
  async getEncryptionKeys (appId) {
    const token = await this.userSettings.getToken()
    const opts = {
      method: 'GET',
      url: `${this.dcAddress}/applications/${appId}/encryptionKeys`,
      json: true,
      // Deliberately longer than the developer connector's own timeout towards the PLC (4s), so that
      // a slow PLC surfaces as the connector's error rather than as a local timeout.
      timeout: 6000,
      headers: { authorization: 'Bearer ' + token },
      resolveWithFullResponse: true,
      simple: false
    ...
    const response = await requestp(opts)
    if (response.statusCode === 404 && response.body && response.body.code === ENCRYPTION_KEYS_UNSUPPORTED_CODE) {
      throw new NotFoundError(t('ERROR_ENCRYPTION_KEYS_NOT_SUPPORTED'))
    }
    if (response.statusCode !== 200) throw new Error(response.body && response.body.message ? response.body.message : t('ERROR_COULD_NOT_GET_ENCRYPTION_KEYS'))

    if (response.headers['x-jwt']) {
      await this.userSettings.setToken(response.headers['x-jwt'])
    }

    return (response.body && response.body.keys) || []
  }

  /**
   * Executes context.user.login()/logout() of a locally running step against the in-flight
   * pipeline request on the pipeline controller. A falsy userId logs the current user out.
   *
   * @param {string} appId
   ...
   * @returns {Promise<void>}
   */
  async userAuthenticate (appId, pipelineRequestId, userId) {
    const token = await this.userSettings.getToken()
    const body = { requestId: pipelineRequestId }
    if (userId) body.userId = userId

    const opts = {
      method: 'POST',
      url: `${this.dcAddress}/applications/${appId}/auth`,
      json: true,
      body,
      // Deliberately longer than the developer connector's own timeout towards the PLC (4s), so that
      // a slow PLC surfaces as the connector's error rather than as a local timeout.
    ...
    if (response.statusCode === 404 && response.body && response.body.code === AUTH_UNSUPPORTED_CODE) {
      throw new NotFoundError(t('ERROR_USER_AUTH_NOT_SUPPORTED'))
    }
    if (response.statusCode !== 200) {
      throw new Error(response.body && response.body.message ? response.body.message : t('ERROR_USER_AUTH_FAILED'))
    }
  }
