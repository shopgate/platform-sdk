...
class DcHttpClient {
  ...
  login (username, password, cb) {
    ...
    request(opts, (err, res, body) => {
      ...
      if (res.statusCode !== 200) return cb(err || new Error(body ? body.message : 'Login failed'))
  ...
  getInfos (infoType, appId, deviceId, cb) {
    ...
    request(opts, (err, res, body) => {
      if (err) return cb(err)
      if (res.statusCode !== 200) return cb(err || new Error(body ? body.message : `could not get ${infoType}`))
    ...
    request(opts, (err, res, body) => {
      ...
      if (res.statusCode !== 200) {
        if (res.statusCode === 404) {
          return cb(new Error(`The application with id '${applicationId}' was not found`))
        }
        return cb(err || new Error(body ? body.message : 'Could not retrieve pipelines'))
      }
