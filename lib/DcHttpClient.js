...
class DcHttpClient {
  ...
  async downloadPipelines (applicationId, trusted, resolved) {
    ...
    if (response.statusCode !== 200) throw new Error(response.body && response.body.message ? response.body.message : 'Pipeline update failed')

    if (response.headers['x-jwt']) {
      await this.userSettings.setToken(response.headers['x-jwt'])
    }

    return response.body.pipelines
  }
