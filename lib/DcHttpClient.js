...
class DcHttpClient {
   ...
   */
  async downloadPipelines (applicationId, trusted, resolved) {
    const token = await this.userSettings.getToken()
    ...
    if (response.statusCode !== 200) {
      if (response.body && response.body.body) response = response.body
      throw new Error(response.body && response.body.message ? response.body.message : 'Pipeline download failed')
    }
  ...
  async generateExtensionConfig (config, applicationId) {
    ...
    if (response.statusCode !== 200) throw new Error(response.body && response.body.message ? response.body.message : 'Could not generate Extension-Config')
  ...
  async pushHooks (config, applicationId) {
    ...
    const hooks = config.steps.filter(step => (step.hooks && step.hooks.length > 0))
