...
class FrontendProcess {
  ...
  /**
   * Returns the webpack project options.
   * @return {Object}
   */
  getWebpackProcessOptions () {
    return {
      env: {
        ...process.env,
  ...
  build () {
    try {
      return fork(join(__dirname, './webpack'), this.getWebpackProcessOptions())
    } catch (error) {
      throw error
    }
  }
  ...
  webpackDevServer () {
    try {
      return fork(join(__dirname, './webpackDevServer'), this.getWebpackProcessOptions())
    } catch (error) {
      throw error
    }
  }
   ...
   */
  linkDependencies () {
    dependencyLinker.init(this.options)
