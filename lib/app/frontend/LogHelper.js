...
class LogHelper {
   ...
   */
  logStartUp () {
    logger.plain(`  Localhost: ${cyan(`http://localhost:${process.env.port}`)}`)
    logger.plain(`  LAN:       ${cyan(`http://${process.env.ip}:${process.env.port}`)}`)
    logger.plain(`  API URL:   ${cyan(`http://${process.env.ip}:${process.env.apiPort}`)}\n`)
