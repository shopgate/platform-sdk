...
class RapidApi {
  ...
  getRequestHeaders (addons = {}) {
    return {
      'sg-application-id': this.settings.shopNumber,
   ...
   */
  requestIds (callback) {
    appStartCommand.p.appIdentifier = `shop:${this.settings.shopNumber.replace('shop_')}`
   ...
   */
  handle (req, res) {
    req.body.vars = { sid: this.settings.sessionId }

    if (req.body.cmds[0].c === 'sendPipelineRequest') req.body.cmds[0].c = 'pipelineRequest'
    if (!req.body.ver) req.body.ver = '9.0'

    const params = {
      url: this.settings.url,
      headers: this.getRequestHeaders({ 'sg-device-id': this.settings.deviceId }),
      body: req.body,
      json: true
    }
