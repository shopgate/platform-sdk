  ...
  init (callback = () => {}) {
    if (!this.settings.shopNumber.startsWith('admin')) {
      return this.requestIds((error, sessionId, deviceId) => {
        ...
        logger.plain(`    SHOP:         ${this.settings.shopNumber.replace('shop_', '')}`)
        logger.plain(`    STARTPAGE IP: ${process.env.startpageIp}`)
        logger.plain(`  RAPID connecting established!\n`)
    ...
    try {
      ...
      request.post(params, (error, response, body) => {
        if (error) {
          console.error(error)
          res.status(500)
          return res.send('Could not connect to RAPID. Please try again later.')
        }
