  ...
  validateEnvironment () {
    return new Promise((resolve, reject) => {
      if (!process.env.ip || !process.env.apiPort) {
        return reject(new Error('No IP addresses and no API port have been specified. Please run the setup first!'))
      }
  ...
  setRoutes () {
    ...
    request.get(`${this.rapidUrl}/status`, (err, res) => {
      ...
      const rapidApi = new RapidApi({
        url: this.rapidUrl,
        shopNumber: this.appId,
        callback: () => {
          // Handle pipeline requests.
          this.server.post('/', rapidApi.handle)
        }
      })
    })
