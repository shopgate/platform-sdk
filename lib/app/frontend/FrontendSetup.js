...
class FrontendSetup {
  constructor () {
    ...
    this.prefix = this.logHelper.getPrefix()
  ...
  async run (showWelcomeMessage = true) {
    if (showWelcomeMessage) {
      this.logHelper.logSetupLogo()
    }

    await this.prompt(this.getQuestions())
  ...
  save (answers) {
    try {
      ...
      logger.plain(`\n${green('SUCCESSS')}: Your ${this.prefix} project is now ready!`)
      logger.plain(`${bold('ATTENTION')}: It may take a few minutes to connect your IP with the ${this.prefix}!\n`)
    } catch (error) {
      throw error
    }
  }
