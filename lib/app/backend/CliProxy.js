...
class CliProxy {
  ...
  async start () {
    ...
    if (applicationId.startsWith('admin_')) {
      ...
      await new Promise((resolve, reject) => {
        request({ method: 'HEAD', url, jar }, (err, res) => {
  ...
  _getIdsFromRapid (rapidUrl, applicationId) {
    let shopNumber
    if (typeof applicationId === 'number' || !isNaN(parseInt(applicationId, 10))) {
      shopNumber = applicationId
    } else {
      const splittedApplicationId = applicationId.split('_')
      shopNumber = splittedApplicationId[1]
    }
  ...
  _getPipelineHandlerFunction (rapidUrl, sessionId, deviceId, shopNumber, trusted) {
    ...
    return (req, res, next) => {
      ...
      const headers = {
        'sg-application-id': `admin_${shopNumber}`,
