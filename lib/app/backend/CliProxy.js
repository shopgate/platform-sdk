...
class CliProxy {
  ...
  _getIdsFromRapid (rapidUrl, applicationId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request(params, (err, res, body) => {
        if (err) reject(err)
        if (!CliProxy._objectPathExists(res, 'body', 'cmds', 0, 'p', 'value') || !res.headers['sg-device-id']) {
          this.logger.info({ res }, t('UNEXPECTED_RESPONSE'))
          return reject(new Error(t('ERROR_NO_RESPONSE_COMMANDS')))
        }
        resolve({
  ...
  _startPipelineServer (port, rapidUrl, sessionId, deviceId, applicationId) {
    return new Promise((resolve, reject) => {
      if (this.server) return reject(new Error(t('ERROR_SERVER_ALREADY_RUNNING')))
      this.server = express()
      this.server.use(express.json())

      this.server.get('/status', (req, res, next) => res.send({ status: 'is running' }))
      this.server.post('/pipelines/:pipelineName', this._getPipelineHandlerFunction(rapidUrl, sessionId, deviceId, applicationId, false))
      this.server.post('/trustedPipelines/:pipelineName', this._getPipelineHandlerFunction(rapidUrl, sessionId, deviceId, applicationId, true))

      this.httpServer = this.server.listen(port, () => {
        this.logger.info(t('PROXY_LISTENING_PORT', { port }))
        resolve(this.server)
      })
    }).then((server) => {
      server.close = this.httpServer.close
      return server
    })
  }
  ...
  _doRequest (params, res, next) {
    request(params, (err, response) => {
      if (err) {
        res.status(500).json(t('ERROR_COULD_NOT_CONNECT_TO_RAPID'))
        return next()
      }
      if (response.statusCode !== 200) {
        const errMsg = response.body ? (response.body.errors || response.body.message) : t('ERROR_PROXYING_REQUEST')
        res.status(response.statusCode).json(errMsg)
        return next()
      }
      if (response.body.cmds[0].p.error) {
        res.status(500).json({ error: response.body.cmds[0].p.error })
        return next()
      }
