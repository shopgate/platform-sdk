...
class BackendProcess extends EventEmitter {
  ...
  /**
   * Restarts are queued, because attaching multiple extensions at once emits one event each and
   * interleaved restarts would orphan watchers that then keep running.
   * @return {Promise<void>}
   */
  async restartStepWatcher () {
    this._stepWatcherRestart = Promise.resolve(this._stepWatcherRestart)
      .catch(() => {})
      .then(async () => {
        // checked inside the queued task, since shutdown can begin while it is still waiting -
        // starting a watcher after disconnect() closed it would leave handles nobody closes
        ...
        try {
          ...
          // the previous watcher is already gone, so giving up here would leave step watching dead
          this.logger.warn(err, t('RETRYING_STEP_WATCHER'))
          await this.executor.startWatcher()
        }
      })

    return this._stepWatcherRestart
  }
  ...
  async disconnect (retry = true) {
    ...
    // a restart queued just before shutdown has to settle first, otherwise it would run after the
    // watcher was closed below
    await Promise.resolve(this._stepWatcherRestart).catch(() => {})
    await this.executor.stopWatcher()
    try {
