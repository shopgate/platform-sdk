...
class BackendProcess {
  constructor (userSettings, appSettings, logger) {
    ...
    this.executor = new StepExecutor(logger, appSettings)
  }
        ...
        (pCb) => {
          ...
          this.socket.on('connect', () => {
            ...
            this.socket.emit('selectApplication', {applicationId}, (err) => {
              ...
              this.socket.emit('resetPipelines', null, err => {
                if (err) return cb(err)
                this.logger.info(`Reset application ${applicationId}`)

                this.socket.emit('reloadPipelines', null, err => {
                  if (err) return cb(err)
                  this.logger.info(`Reloaded application ${applicationId}`)
