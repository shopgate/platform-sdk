...
class BackendProcess {
    ...
    this.socket.on('connect', () => {
      ...
      this.socket.emit('selectApplication', {applicationId: AppSettings.getInstance().getId()}, (err) => {
        ...
        this.extensionWatcher.on('attach', (extensionId) => {
          ...
          this.socket.emit('registerExtension', {extensionId}, () => {
            logger.info(`Extension ${extensionId} attached`)
          })
        })

        this.extensionWatcher.on('detach', (extensionId) => {
          logger.info(`Extension ${extensionId} removed`)

          this.socket.emit('deregisterExtension', {extensionId}, () => {
  ...
  stepCall (data, cb) {
    console.log('Execute')
    this.executor.execute(data.input, data.stepMetaData, (err, output) => {
      console.log('stepCallCallback')
      cb(err, output)
    })
  }
