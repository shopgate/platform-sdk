    ...
    this.socket.on('connect', () => {
      ...
      this.socket.emit('selectApplication', {applicationId: AppSettings.getInstance().getId()}, (err) => {
        ...
        this.extensionWatcher.on('attach', (extension) => {
          ...
          this.socket.emit('registerExtension', {extensionId: extension}, (err) => {
            if (err) return logger.error(`Error while attaching the extension ${extension}: ${err.message}`)
        ...
        this.extensionWatcher.on('detach', (extension) => {
          ...
          this.socket.emit('deregisterExtension', {extensionId: extension}, (err) => {
            if (err) return logger.error(`Error while detaching the extension ${extension}: ${err.message}`)
