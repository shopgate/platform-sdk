...
const DcResponseHandler = require('./context/DcResponseHandler')
...
let appSettings
let storage
let dcResponseHandler
const logger = new Logger();

(async function ()  /* istanbul ignore next */ {
  ...
  dcResponseHandler = DcResponseHandler.getInstance()
})().then(() => /* istanbul ignore next */ {
  process.on('message', DcResponseHandler.handle)
  process.on('message',
    /**
    ...
    async (message) => {
      if (message.type === 'dcResponse') {
        // see DcResponseHandler.handle() for handling of dcResponse type messages
        return
      }
      ...
      const context = new Context(storage, config, dcResponseHandler, message.stepMeta.id, message.stepMeta.meta, logger)
