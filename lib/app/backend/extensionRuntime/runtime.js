...
const publicKeyRegistryFromEnv = require('./context/publicKeyRegistryFromEnv')
...
(async function () {
  ...
  publicKeyRegistry = publicKeyRegistryFromEnv(process.env, logger)
})().then(() => {
    ...
    async (message) => {
      ...
      function sendOutput (err, output) {
        ...
        if (err) {
          ...
          err = errio.toObject(err, { stack: true })
        }
