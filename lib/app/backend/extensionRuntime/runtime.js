...
function getLogObject (errorObj, obj) {
  let logObj = {
    context: 'runtime'
  }

  if (errorObj) logObj = Object.assign(logObj, {err: errorObj})
  if (obj) logObj = Object.assign(logObj, obj)
...
function onMessage (message) {
  ...
  logger.info(getLogObject(), `Starting step execution of ${message.stepMeta.path}`)

  function sendOutput (err, output) {
    logger.info(getLogObject(), `Step execution of ${message.stepMeta.path} done`)
  ...
  if (!fsEx.pathExistsSync(stepFile)) {
    ...
    logger.error(getLogObject(err), err.message)
    return sendOutput(err)
  }

  process.on('uncaughtException', (err) => {
    logger.error(getLogObject(err), err.stack)
    sendOutput(err)
