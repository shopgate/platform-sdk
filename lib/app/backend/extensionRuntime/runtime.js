...
const PublicKeyRegistry = require('./context/PublicKeyRegistry')
...
let mapStorage
let dcRequest
let publicKeyRegistry

function buildPublicKeyRegistry () {
  try {
    return new PublicKeyRegistry(JSON.parse(process.env.ENCRYPTION_PUBLIC_KEYS || '[]'))
  } catch (err) {
...
(async function () {
  ...
  publicKeyRegistry = buildPublicKeyRegistry()
})().then(() => {
  ...
  process.on('message',
    ...
    async (message) => {
      ...
      const extensionPath = path.join(extensionsPath, attachedExtensions[message.stepMeta.id].path)
      const absExtPath = path.join(extensionPath, 'extension')
      const stepFile = path.join(absExtPath, message.stepMeta.path.substr(message.stepMeta.id.length))
      const configPath = utils.getBackendConfigPath(appSettings, extensionPath)

      let config = {}
      try {
        config = await fsEx.readJson(configPath, { throws: false }) || {}
      } catch (err) {
        logger.warn(t('ERROR_NO_EXTENSION_CONFIG', { path: configPath }))
      }
      ...
      const context = new Context(storage, mapStorage, config, dcRequest, message.stepMeta.id, message.stepMeta.meta, logger, publicKeyRegistry)
