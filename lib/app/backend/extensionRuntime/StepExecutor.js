...
// Steps are js, but they also read json next to them. Dependency manifests are excluded, so that
// installing inside an extension does not restart the runtime.
const STEP_FILE = /\.(js|json)$/
const NOT_A_STEP_FILE = /^(package|package-lock)\.json$/
  ...
  async onMessage (msg) {
    switch (msg.type) {
      ...
      case 'output':
        ...
        break
      case 'dcRequest':
        // A failure must be sent back as dcResponse.error; an unhandled rejection here would
        // leave the step's promise pending until it runs into the step timeout
        try {
          const { resourceName, appId, deviceId, pipelineRequestId, userId } = msg.dcRequest
          const info = resourceName === 'userAuth'
            ? await this.dcHttpClient.userAuthenticate(appId, pipelineRequestId, userId)
            : await this.dcHttpClient.getInfos(resourceName, appId, deviceId)
          this.childProcess.send({ type: 'dcResponse', requestId: msg.dcRequest.requestId, info })
        } catch (err) {
