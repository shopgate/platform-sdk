...
class StepExecutor {
  ...
  onMessage (msg) {
    if (msg.type === 'log') {
      ...
      const message = msg.message || 'No message given'
      delete msg.level
      delete msg.message
      delete msg.type
      return this.log[level](msg, message)
  ...
  execute (input, stepMeta, cb) {
    ...
    stepMeta.callId = this.calls++
    this.openCalls[stepMeta.callId] = cb
    this.child.send({input, stepMeta})
