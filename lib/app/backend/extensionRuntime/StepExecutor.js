...
class StepExecutor {
  ...
  constructor (log, appSettings, dcHttpClient, inspect, encryptionKeys = { keys: [], unavailableReason: '' }) {
    ...
    this.watcherOptions = {
      ignored: (filePath, stats) => IGNORED_PATH.test(filePath) ||
        (!!stats && stats.isFile() && path.extname(filePath) !== '.js'),
      ignoreInitial: true
    }
   ...
   * extensions folder. Folders that do not exist are left out on purpose: combined with an ignore
   * function they would silently disable the entire watcher.
  ...
  async _getStepFolders () {
    ...
    const folders = Object.values(attachedExtensions)
      .map(extension => path.join(extensionsFolder, extension.path, STEP_FOLDER))
    const existing = await Promise.all(folders.map(folder => fsEx.pathExists(folder)))

    return folders.filter((folder, index) => existing[index])
  }
