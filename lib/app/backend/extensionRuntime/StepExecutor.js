...
const { PIPELINES_FOLDER, EXTENSIONS_FOLDER, UNAVAILABLE_UNSUPPORTED, UNAVAILABLE_FAILED } = require('../../Constants')
...
class StepExecutor {
  ...
  /**
   * Fetches the public encryption keys so the runtime can encrypt locally. Never blocks startup: on
   * failure it resolves to an empty list plus the reason why, which the runtime turns into an
   * explanatory error when a step calls `context.encrypt`.
  ...
  async _loadEncryptionKeys () {
    try {
      ...
      return { keys: await this.dcHttpClient.getEncryptionKeys(appId), unavailableReason: '' }
    } catch (err) {
  ...
  start () {
    return new Promise((resolve, reject) => {
      ...
      this._loadEncryptionKeys().then(({ keys, unavailableReason }) => {
        // env is passed via the effective options object (2nd arg) and inherits the current env.
        // Both variables are always set so that a stale value can never leak in from the outside.
        const forkOptions = {
          execArgv,
          env: {
            ...process.env,
            ENCRYPTION_PUBLIC_KEYS: JSON.stringify(keys),
            ENCRYPTION_KEYS_UNAVAILABLE: unavailableReason
          }
        }
        this._startChild(forkOptions, resolve)
      }).catch(reject)
    })
  }

  _startChild (forkOptions, resolve) {
    this.childProcess = fork(program, forkOptions, childProcessOptions)
