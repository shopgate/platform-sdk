...
class StepExecutor {
  ...
  /**
   * Fetches the public encryption keys so the runtime can encrypt locally. Never blocks startup:
   * on failure it resolves to an empty list and `context.encrypt` will report unknown keys.
   * @returns {Promise<Array<{alias: string, publicKeyPem: string}>>}
   */
  async _loadEncryptionKeys () {
    try {
      const appId = await this.appSettings.getId()
      return await this.dcHttpClient.getEncryptionKeys(appId)
    } catch (err) {
      this.log.warn(t('ERROR_COULD_NOT_LOAD_ENCRYPTION_KEYS'))
      this.log.debug({ err }, t('ERROR_COULD_NOT_LOAD_ENCRYPTION_KEYS'))
      return []
    }
  }
  ...
  start () {
    return new Promise((resolve, reject) => {
      ...
      this._loadEncryptionKeys().then((encryptionKeys) => {
        // env is passed via the effective options object (2nd arg) and inherits the current env.
        const forkOptions = { execArgv, env: { ...process.env, ENCRYPTION_PUBLIC_KEYS: JSON.stringify(encryptionKeys) } }
