...
class StepExecutor {
  ...
  watch () {
    chokidar.watch('extensions', {cwd: process.cwd()}).on('all', () => {
  ...
  execute (input, stepMeta, cb) {
    if (!this.childProcess) return cb(new Error('Process not running, please restart or try again in a few seconds'))
