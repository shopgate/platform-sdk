...
class StepExecutor {
  ...
  start () {
    return new Promise((resolve, reject) => {
      ...
      const childProcessParameter = []
      if (this.appSettings.hasOwnProperty('inspect') && this.appSettings.inspect) {
        childProcessParameter.push('--inspect')
      }

      this.childProcess = fork(program, {execArgv: childProcessParameter}, childProcessOptions)
  ...
  startWatcher () {
    return new Promise((resolve, reject) => {
      const extensionsFolder = path.join(this.appSettings.getApplicationFolder(), 'extensions', '**', 'extension', '*')
      this.watcher = chokidar.watch(extensionsFolder, this.watcherOptions)
