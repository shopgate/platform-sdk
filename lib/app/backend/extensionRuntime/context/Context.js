...
function userMapStorageInterface (mapStorage, appId, extensionId, userId) {
  if (userId) return mapStorageInterface(mapStorage, `${appId}/${extensionId}/users/mapStorage/${userId}/`)
  const error = new Error('user not logged in')
...
class Context {
  ...
  constructor (storage, mapStorage, config, dcRequester, extensionId, meta, log) {
    ...
    this.storage = {
      ...
      map: {
        extension: mapStorageInterface(mapStorage, `${meta.appId}/${extensionId}/mapStorage/`),
        device: mapStorageInterface(mapStorage, `${meta.appId}/${extensionId}/mapStorage/${meta.deviceId}/`),
