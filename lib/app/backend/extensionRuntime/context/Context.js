...
class Context {
   ...
   * @param {IntercomponentStorage} ics
   * @param {object} config
   ...
   * @param {object} log
   */
  constructor (storage, ics, config, extensionId, meta, log) {
    this.meta = meta
    this.config = config
    this.storage = {
    ...
    this.app = {
      getInfo: (cb) => ics.getInfos('appinfos', meta.deviceId, cb)
    }
    this.device = {
      getInfo: (cb) => ics.getInfos('deviceinfos', meta.deviceId, cb)
    }
