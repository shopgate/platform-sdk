...
class Context {
   ...
   * @param {object} log
   * @param {PublicKeyRegistry} [publicKeyRegistry]
   */
  constructor (storage, mapStorage, config, dcRequester, extensionId, meta, log, publicKeyRegistry) {
    this.meta = meta
    this.config = config
    this.storage = {
    ...
    // Forwarded via the developer connector to the plc, which runs the same login/logout logic as
    // its auth step against the in-flight pipeline request. The plc rejects login (403) for steps
    // of untrusted pipelines; logout is always allowed. meta.userId of this context stays a
    // snapshot; only steps executed after a login/logout see the updated userId.
    this.user = {
      login: async (userId) => {
        if (!userId) throw new Error(t('ERROR_LOGIN_REQUIRES_USER_ID'))
        if (!meta.requestId) throw new Error(t('ERROR_NO_PIPELINE_REQUEST'))
        await dcRequester.requestUserAuth(meta.appId, meta.requestId, userId)
      },
      logout: async () => {
        if (!meta.requestId) throw new Error(t('ERROR_NO_PIPELINE_REQUEST'))
        await dcRequester.requestUserAuth(meta.appId, meta.requestId, null)
      }
    }
    ...
    // Encrypt locally with the public keys mirrored from the pipeline controller so plaintext never
    // leaves the developer's machine. Signature and behaviour are identical to the production PLC.
    this.encrypt = async (keyName, buffer) => {
      if (!publicKeyRegistry) throw new Error(`unknown public key "${keyName}"`)
      return publicKeyRegistry.encrypt(keyName, buffer)
    }
  }
}
