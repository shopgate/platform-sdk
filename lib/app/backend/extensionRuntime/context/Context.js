...
class Context {
  ...
  constructor (storage, dcHttpClient, config, extensionId, meta, log) {
    ...
    this.app = {
      getInfo: (cb) => { dcHttpClient.getInfos('appinfos', meta.appId, meta.deviceId).then((appinfos) => cb(appinfos)) }
    }
    this.device = {
      getInfo: (cb) => { dcHttpClient.getInfos('deviceinfos', meta.appId, meta.deviceId).then((deviceinfos) => cb(deviceinfos)) }
    }
