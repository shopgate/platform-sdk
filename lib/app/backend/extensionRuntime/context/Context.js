...
// Mirrors the pipeline controller: the encrypted Buffer is returned directly but decorated with
// then/catch/finally so callers can either use the return value synchronously or `await` it.
function getAwaitableEncryptBuffer (buffer) {
  const promise = Promise.resolve(Buffer.from(buffer))
...
class Context {
  ...
  constructor (storage, mapStorage, config, dcRequester, extensionId, meta, log, publicKeyRegistry) {
    ...
    this.encrypt = (keyName, buffer, cb) => {
      if (!publicKeyRegistry) throw new Error(`unknown public key "${keyName}"`)
      if (typeof cb !== 'function') return getAwaitableEncryptBuffer(publicKeyRegistry.encrypt(keyName, buffer))

      process.nextTick(() => {
        try {
          cb(null, publicKeyRegistry.encrypt(keyName, buffer))
        } catch (err) {
          cb(err)
        }
      })
