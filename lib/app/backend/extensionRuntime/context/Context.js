...
const requestDefaults = { simple: false, resolveWithFullResponse: true }
...
class Context {
  ...
  constructor (storage, mapStorage, config, dcRequester, extensionId, meta, log) {
    ...
    this.tracedRequest = (name, options = {}) => {
      if (!options.log) return requestPromise

      return request.defaults((requestOptions, cb) => {
        ...
        }, 'Extension sending a context.tracedRequest.')

        if (typeof cb === 'function') {
          return request(requestOptions, (err, data) => {
            if (err) {
              this.log.error(err)
              return cb(err)
            }
            ...
            }, 'Extension received response from a context.tracedRequest.')

            cb(null, data)
          })
        } else {
          return requestPromise({ ...requestDefaults, ...requestOptions })
            .then((data) => {
              this.log[options.logLevel]({
                tracedRequestInfo: {
                  traceId,
