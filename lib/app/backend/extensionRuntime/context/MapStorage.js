...
class MapStorage {
  ...
  getItem (mapId, key, cb) {
    ...
    this._queue.push({ operation: 'getElement', mapId, key, cb }, err => { if (err) throw err })
  }
  ...
  setItem (mapId, key, value, cb) {
    ...
    this._queue.push({ operation: 'setElement', key, value, cb }, err => { if (err) throw err })
  }
  ...
  delItem (mapId, key, cb) {
    ...
    this._queue.push({ operation: 'delElement', key, cb }, err => { if (err) throw err })
  }
  ...
  async _handleStorageOperation (task) {
    ...
    switch (task.operation) {
      ...
      case 'set':
        try {
          if (task.value !== Object.create(task.value) || Array.isArray(task.value)) {
      ...
      case 'getElement':
        if (!storage[task.mapId]) {
          return task.cb(new Error('Cannot get map item from non-existing map with ID "' + task.mapId + '".'))
        }
      ...
      case 'delElement':
        if (!storage[task.mapId]) {
          return task.cb(new Error('Cannot delete map item from non-existing map with ID "' + task.mapId + '".'))
        }
