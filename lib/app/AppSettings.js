...
class AppSettings {
  ...
  async getId () {
    ...
    if (!data.id) throw new Error('The current folder seems not to be an sgconnect project. Please run sgconnect init first.')
    return data.id
  }
  ...
  async attachExtension (pathName, extensionInfo, force) {
    ...
    if (!force && extensions[extensionInfo.id]) return logger.warn(`Extension '${extensionInfo.id} (${pathName}) is already attached`)
