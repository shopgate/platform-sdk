...
class AppSettings {
  ...
  attachExtension (pathName, extensionInfo, force) {
    if (!force && this.attachedExtensions[extensionInfo.id]) {
      throw new Error(`Extension '${extensionInfo.id} (${pathName}) is already attached`)
    }

    logger.info(`Attached ${extensionInfo.id} (${pathName}, trusted: ${extensionInfo.trusted})`)
    this.attachedExtensions[extensionInfo.id] = {path: pathName, trusted: extensionInfo.trusted}
    return this
  }
   ...
   */
  init () {
    if (!fsEx.pathExistsSync(this.settingsFile) || !fsEx.pathExistsSync(this.attachedExtensionsFile)) {
      throw new Error('This current folder does not contain any application data. Please run `sgcloud init` first.')
    }
