...
class AppSettings {
  constructor () {
    let applicationFolder = process.cwd()

    try {
      if (!process.env.APP_PATH) {
        applicationFolder = findRoot(process.cwd(), (dir) => fsEx.existsSync(path.join(dir, SETTINGS_FOLDER, 'app.json')))
      }
    } catch (err) {
      // NOTE: workaround, find root would print "package.json not found in path"
      err.message = "cloudn't find application folder upwards"
      logger.debug(err)
    }

    this.applicationFolder = process.env.APP_PATH || applicationFolder
