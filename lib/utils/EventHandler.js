...
module.exports = {
  ...
  extensionConfigChanged: async (config, appSettings, dcHttpClient) => {
    ...
    try {
      ...
      return logger.error(`Could not validate config for '${config.file.id}': ${err.message}`)
    }
    ...
    try {
      ...
      logger.error(`Could not generate config for '${config.file.id}': ${err.message}`)
      logger.debug(err)
    }
    ...
    try {
      ...
      logger.error(`Could not update hooks triggered by change of '${config.file.id}': ${err.message}`)
      logger.debug(err)
    }
  }
}
