...
module.exports = {
   ...
   * @param {string} source
   */
  extensionConfigChanged: async (config, appSettings, dcHttpClient, source = 'backend') => {
    const appId = await appSettings.getId()
    const target = source === 'frontend' ? 'frontend' : 'all'
    ...
    try {
      await updateExtensionConfig(config, appId, dcHttpClient, appSettings, target)
