...
class LogStream {
  ...
  write (rec) {
    ...
    // Set time
    const time = stylize(rec.time.toLocaleString(this.locale), 'gray')
    ...
    if (rec.err && rec.err.stack) details.push(indent(rec.err.stack))
    ...
    const extrasStr = stylize((extras.length ? ' (' + extras.join(', ') + ')' : ''), 'gray')
