...
async function getDirectories (dirPath) {
  return filterDir(dirPath, async entry => {
    return (await fsEx.lstat(entry)).isDirectory()
  })
}
...
async function validateExtensionConfig (file) {
  await validator(path.join(file.path, 'extension-config.json'))
}

async function writeLocalPipelines (dcHttpClient, appId, pipelinesFolder, trustedPipelinesFolder) {
  logger.info('Preprocessing pipelines')
  await Promise.all([
    _getAndSavePipelines(dcHttpClient, appId, false, false, pipelinesFolder),
    _getAndSavePipelines(dcHttpClient, appId, true, false, trustedPipelinesFolder)
  ])
  await Promise.all([
    _getAndSavePipelines(dcHttpClient, appId, false, true, path.join(pipelinesFolder, 'resolved')),
    _getAndSavePipelines(dcHttpClient, appId, true, true, path.join(trustedPipelinesFolder, 'resolved'))
  ])
}
 ...
 */
async function _getAndSavePipelines (dcHttpClient, appId, trusted, resolved, destination) {
  const pipelines = await dcHttpClient.downloadPipelines(appId, trusted, resolved)
  return _writePipelines(pipelines, destination)
}
