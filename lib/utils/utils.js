...
async function resetProject () {
  ...
  await Promise.all(extensionDirs.map(async extensionDir => removeIfExists(path.join(extensionDir, 'extension', 'config.json'))))

  const themesDir = path.join(appPath, THEMES_FOLDER)
  const themeDirs = await getDirectories(themesDir)
  await Promise.all(themeDirs.map(async themeDir => removeIfExists(path.join(themeDir, 'config', 'app.json'))))
  await Promise.all(themeDirs.map(async themeDir => removeIfExists(path.join(themeDir, 'config', 'components.json'))))

  const pipelinesDir = path.join(appPath, PIPELINES_FOLDER)
  const pipelineFiles = await getFiles(pipelinesDir)
  await Promise.all(pipelineFiles.map(async pipelineFile => removeIfExists(pipelineFile)))
  ...
  await Promise.all(trustedPipelineFiles.map(async trustedPipelineFile => removeIfExists(trustedPipelineFile)))
}
