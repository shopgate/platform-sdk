...
async function generateComponentsJson (appSettings) {
  ...
  const filtered = themes.filter(theme => !isKnownHiddenDirectory(theme))

  if (!filtered.length) return Promise.resolve()

  logger.info(`Generating components.json for ${filtered}`)

  // write components.json for each theme
  return Promise.all(filtered.map(async theme => {
...
function isKnownHiddenDirectory (name) {
  const blacklist = ['node_modules', 'coverage', 'bower_components', 'typings', 'lib-cov']
  return name.startsWith('.') || blacklist.includes(name)
}
