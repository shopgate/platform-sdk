 ...
 */
/**
 * Writes json only when it differs from what is already on disk. Rewriting an unchanged config
 * would still update its mtime, which makes the file watchers reload for nothing.
 * @param {string} file
 * @param {Object} content
 * @return {Promise<boolean>} whether the file was written
 */
async function writeJsonIfChanged (file, content) {
  const current = await fsEx.readJson(file, { throws: false }).catch(() => null)
  if (current !== null && isDeepStrictEqual(current, content)) return false

  await fsEx.outputJson(file, content, { spaces: 2 })
  return true
}
...
async function updateExtensionConfig (config, appId, dcHttpClient) {
  try {
    ...
    if (await fsEx.pathExists(backendPath)) {
      if (await writeJsonIfChanged(path.join(backendPath, 'config.json'), backendConfig)) {
        logger.info(`Updated extension config for backend ${config.file.id}`)
      }
    }

    if (await fsEx.pathExists(frontendPath)) {
      if (await writeJsonIfChanged(path.join(frontendPath, 'config.json'), frontendConfig)) {
