...
class ExtensionConfigWatcher extends EventEmitter {
  constructor (options, watchFolder) {
    super()
    const extensionFolder = process.env.APP_PATH ? path.join(process.env.APP_PATH, EXTENSION_FOLDER) : EXTENSION_FOLDER
    this.options = options || {}
    this.options.ignoreInitial = true
    this.watchFolder = watchFolder || extensionFolder
  }

  start () {
    this.watcher = chokidar.watch(path.join(this.watchFolder, '**', 'extension-config.json'), this.options).on('all', (event, path) => {
