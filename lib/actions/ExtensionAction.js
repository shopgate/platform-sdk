...
const BOILERPLATE_URL = 'https://github.com/shopgate/cloud-sdk-boilerplate-extension/archive/master.zip'
...
class ExtensionAction {
  ...
  renameBoilerPlate (userInput, defaultPath, extensionsFolder, state) {
    return () => {
      ...
      return new Promise((resolve, reject) => {
        fsEx.move(defaultPath, ePath, {overwrite: false}, (err) => {
  ...
  removePlaceHolders (userInput, state) {
    return () => {
      ...
      return replace({
        ...
        .then(logger.debug('Removing placeholders ... done'))
        .catch(err => {
    ...
    return () => {
      ...
      if (userInput.trusted) {
        ...
        .then((exConf) => {
          ...
          return fsEx.writeJson(exConfPath, exConf)
        })
   ...
   * @param {object} state
   */
  installFrontendDependencies (userInput, state) {
    return () => {
  ...
  createExtension (types, options) {
    ...
    this.getUserInput(options, types, userInput)
      ...
      .catch((err) => {
        logger.error('An error occured while creating the extension')
        logger.error(err)
