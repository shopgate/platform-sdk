...
class ExtensionAction {
  ...
  async _packExtension (extensionPath) {
    ...
    try {
      ...
      ignorePatterns.add(parseGitignore(gitignoreContents))
    } catch (err) {
      logger.debug(t('NO_GITIGNORE_IN_DIRECTORY', { directory: extensionPath }))
      ignorePatterns.add('node_modules')
    }
    ignorePatterns.add(['.git', '.gitignore'])

    const tempFile = fsEx.createWriteStream(archivePath)
    const tarStream = tar.pack(extensionPath, {
      ignore (name) {
        const stats = fsEx.statSync(name)
        name = path.relative(extensionPath, name)

        if (stats.isDirectory() && !name.endsWith('/')) {
          // For directories the path needs to have a trailing slash,
          // otherwise they would not match if the ignore rule has a trailing slash.
          // The slash needs to be added AFTER making the path relative,
          // because that would remove it again.
          name += '/'
        }

        return ignorePatterns.ignores(name)
      }
    })
