...
class ExtensionAction {
  ...
  async detachExtensions ({ extensions = [] }) {
    ...
    for (const processedExtension of processedExtensions) {
      ...
      await utils.removeGeneratedBackendConfig(
        this.appSettings,
        path.join(this.appSettings.getApplicationFolder(), EXTENSIONS_FOLDER, processedExtension.extensionFolderName)
      )
    }
  }
  ...
  async manageExtensions () {
    ...
    for (let i = 0, l = goneExtensions.length; i < l; i++) {
      ...
      await utils.removeGeneratedBackendConfig(
        this.appSettings,
        path.join(this.appSettings.getApplicationFolder(), EXTENSIONS_FOLDER, attachedExtensions[goneExtensions[i]].path)
      )
    }
