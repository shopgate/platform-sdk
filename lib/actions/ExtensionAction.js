...
class ExtensionAction {
  ...
  attachExtensions (extensions) {
    ...
    extensions.forEach(ext => {
      ...
      this.settings.attachExtension(ext, id, force)
    })
    this.settings.save()
  }
  ...
  getAllExtensions () {
    ...
    const exts = []
    files.forEach(file => {
      if (this.getExtensionId(file)) exts.push(file)
    })
    return exts
  }
  ...
  getExtensionId (extensionName) {
    ...
    throw new Error(`Config file of '${extensionName}' is invalid or not existent`)
  }
}
