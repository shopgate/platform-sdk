...
class ExtensionAction {
  ...
  static register (caporal, appSettings) {
    ...
    caporal
      ...
      .action((args, options) => new ExtensionAction(appSettings).attachExtensions(args, options))
    ...
    caporal
      ...
      .action((args, options) => new ExtensionAction(appSettings).detachExtensions(args, options))
  }
  ...
  attachExtensions ({extensions = []}) {
    ...
    extensions.forEach((extensionFolderName) => {
      ...
      this.appSettings.attachExtension(extensionFolderName, extensionInfo, force)
    })
  }
  ...
  detachExtensions ({extensions = []}) {
    ...
    if (!extensions.length) {
      this.appSettings.detachAllExtensions()
    } else {
      extensions.forEach((extensionFolderName) => {
        const extensionInfo = this._getExtensionInfo(extensionFolderName, true)
        if (!extensionInfo) {
        ...
        this.appSettings.detachExtension(extensionInfo.id)
      })
    }
  }
