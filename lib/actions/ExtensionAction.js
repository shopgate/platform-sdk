...
class ExtensionAction {
  ...
  static register (caporal, appSettings, userSettings, dcHttpClient) {
    ...
    caporal
      ...
      .option('-t, --preprocess-timeout <ms>', 'Preprocessing timeout, milliseconds', caporal.INT, 18e4)
  ...
  async uploadExtension (args = {}, options = {}) {
    ...
    const archivePath = await utils.runSpinner(`Packing ${extensionId}@${extensionVersion}`, () => this._packExtension(extensionPath))
    logger.debug(`Packed ${extensionId}@${extensionVersion} into ${archivePath}`)

    try {
      await utils.runSpinner(`Uploading ${extensionId}@${extensionVersion}`, () =>
        this.dcHttpClient.uploadExtensionFile(extensionId, extensionVersion, archivePath, options.force)
      )
    } catch (err) {
      if (err instanceof NotFoundError && err.message === 'Version not found' && !options.force) {
        ...
        if (createVersion) {
          await utils.runSpinner(`Uploading ${extensionId}@${extensionVersion}`, () =>
            this.dcHttpClient.uploadExtensionFile(extensionId, extensionVersion, archivePath, true)
          )
        } else {
          throw new Error(`Version ${extensionVersion} of ${extensionId} does not exist`)
        }
      } else if (err instanceof NotFoundError && err.message.toLowerCase() === 'extension not found') {
        throw new Error(`${isTheme ? 'Theme' : 'Extension'} ${extensionId} was not found in Shopgate Developer Center`)
      } else {
        throw err
      }
    }
    logger.debug(`Uploaded ${extensionId}@${extensionVersion}`)

    await utils.runSpinner(`Preprocessing ${extensionId}@${extensionVersion}`, () => {
    ...
    logger.debug(`Preprocessing of ${extensionId}@${extensionVersion} finished`)

    logger.debug(`Deleting ${archivePath}`)
    await fsEx.remove(archivePath)

    logger.plain(`${isTheme ? 'Theme' : 'Extension'} ${extensionId}@${extensionVersion} successfully uploaded`)
  }
  ...
  async _resolveExtensionInfo (extensionDir, isTheme = false) {
    ...
    if (!extensionDir) {
      ...
      if (choices.length === 0) {
        ...
        throw new Error(`There are no ${isTheme ? 'themes' : 'extensions'} in './${relativeExtensionsDirectory}'`)
      }
    ...
    const extension = extensionPropeties.find(({ dir }) => dir === extensionDir)

    if (!extension) {
      throw new Error(`${isTheme ? 'Theme' : 'Extension'} directory ${extensionDir} does not exist`)
    }

    extensionInfo.extensionId = extension.id
    extensionInfo.extensionDir = extension.dir

    if (isTheme && extension.isTheme !== true) {
      logger.warn(`${extension.id} exists but is not marked as theme (type: "theme")`)
    }
