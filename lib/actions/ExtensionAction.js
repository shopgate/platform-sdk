...
class ExtensionAction {
  ...
  /**
   * Sets a new version in extension-config.json, commits it and creates a git tag with the same name.
  ...
  async versionExtension (args = {}) {
    ...
    if (isGitRepository) {
      ...
      if (await this._git(extensionPath, ['tag', '-l', version])) {
        throw new Error(t('ERROR_GIT_TAG_EXISTS', { tag: version, directory: resolvedDir }))
      }
    }
    ...
    await this._git(extensionPath, ['tag', '-a', version, '-m', version])

    logger.plain(t('VERSION_SET_AND_TAGGED_SUCCESSFULLY', { extensionId, version }))
  }
