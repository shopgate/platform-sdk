    ...
    caporal
      ...
      .action(async (args, options) => {
        try {
          ...
          if (process.env.INTEGRATION_TEST) return process.exit(0)
        } catch (err) {
  ...
  _getUserInput (options, types = [], externalUserInput) {
    ...
    if (userInput.trusted === null) {
      questions.push({
        ...
        default: 'no',
        when: (answers) => {
          const isBackendExtension = (answers.types && answers.types.includes('backend')) || types.includes('backend')
          // Consider it unnecessary if there isn't a type neigher or its only frontend
          return isBackendExtension
        }
      })
    }
