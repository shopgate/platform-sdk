...
class ExtensionAction {
  ...
  async manageExtensions () {
    ...
    if (extensionPropeties.length === 0) {
      return logger.warn(`There are no extensions in folder './${EXTENSIONS_FOLDER}'`)
    }
  ...
  async uploadExtension (args = {}, options = {}) {
    ...
    const isTheme = args.isTheme === true
    const extensionsDirectory = path.join(this.appSettings.getApplicationFolder(), EXTENSIONS_FOLDER)

    const extensionPropeties = await this._getAllExtensionProperties()
    if (extensionPropeties.length === 0) {
      return logger.warn(`There are no extensions in folder './${path.relative(process.cwd(), extensionsDirectory)}'`)
    }
    ...
    if (args.extension) {
      ...
      if (!extension) {
        throw new Error(`${isTheme ? 'Theme' : 'Extension'} folder ${args.extension} does not exist`)
      }
      ...
      if (choices.length === 0) {
        return logger.warn(`There are no themes in folder './${path.relative(process.cwd(), extensionsDirectory)}'`)
      }
    ...
    let extensionConfig
    try {
      extensionConfig = await fsEx.readJSON(path.resolve(extensionPath, 'extension-config.json'))
    } catch (err) {
      if (err.code === 'ENOENT') {
    ...
    try {
      ...
      logger.debug(`.gitignore does not exist in the ${isTheme ? 'theme' : 'extension'} folder. Using defaults`)
    ...
    logger.debug('Packing folder into tar archive...')
