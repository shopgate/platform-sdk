...
class ExtensionAction {
  ...
  attachExtensions ({extensions = []}) {
    ...
    extensions.forEach((extensionFolderName) => {
      ...
      if (extensionInfo.components && extensionInfo.components.length > 0) {
        this._generateComponentsJson(extensionFolderName, extensionInfo.id, extensionInfo.components)
      }
    })
  }
  ...
  _checkIfExtensionExists (extensionName) {
    const extensionsFolder = path.join(this.settings.getApplicationFolder(), AppSettings.EXTENSIONS_FOLDER, extensionName.replace('/', '-'))
   ...
   */
  _generateComponentsJson (extensionName, extensionId, eCComponents) {
    const components = {}
    ...
    const extensionsFolder = path.join(this.settings.getApplicationFolder(), AppSettings.EXTENSIONS_FOLDER)
    const file = path.join(extensionsFolder, extensionName, 'frontend', 'components.json')

    try {
      fsEx.writeJSONSync(file, components)
    } catch (e) {
      logger.warn(`writing "${file}" failed`)
    }
  }
}
