...
class ExtensionAction {
  ...
  async uploadExtension (args = {}, options = {}) {
    ...
    await utils.runSpinner(`Uploading ${extensionId}@${extensionVersion}`, async () => {
      try {
        await this.dcHttpClient.uploadExtensionFile(extensionId, extensionVersion, archivePath, options.force)
      } catch (err) {
        if (err instanceof NotFoundError && err.message === 'Version not found' && !options.force) {
          ...
          if (createVersion) {
            await this.dcHttpClient.uploadExtensionFile(extensionId, extensionVersion, archivePath, true)
          } else {
