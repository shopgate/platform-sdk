  ...
  static register (caporal, appSettings, userSettings) {
    caporal
      ...
      .action(async (args, options) => {
        try {
          await new ExtensionAction(appSettings, userSettings).createExtension(args, options)
        } catch (err) {
    ...
    caporal
      ...
      .action(async (args) => {
        try {
          await new ExtensionAction(appSettings, userSettings).attachExtensions(args)
        } catch (err) {
          logger.error(err.message)
          process.exit(1)
        }
    ...
    caporal
      ...
      .action(async (args) => {
        try {
          await new ExtensionAction(appSettings, userSettings).detachExtensions(args)
        } catch (err) {
          logger.error(err.message)
          process.exit(1)
        }
  ...
  async attachExtensions ({extensions = []}) {
    ...
    return Promise.all(extensions
      ...
      .map(async processedExtension => {
        ...
        utils.generateComponentsJson(this.appSettings)
      })
    )
  }
