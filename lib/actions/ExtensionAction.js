...
class ExtensionAction {
  ...
  async _resolveExtensionInfo (extensionDir, isTheme = false) {
    ...
    try {
      ...
      let errorMessage
      if (!extensionConfig) {
        errorMessage = `Invalid JSON in ./${relativeExtensionsConfigPath}`
      } else if (!extensionConfig.id) {
        errorMessage = `There is no 'id' property in ./${relativeExtensionsConfigPath}`
      } else if (!extensionConfig.version) {
        errorMessage = `There is no 'version' property in ./${relativeExtensionsConfigPath}`
      }
      ...
      if (isTheme && extensionConfig.isTheme !== true) {
