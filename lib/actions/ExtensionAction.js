...
class ExtensionAction {
  ...
  static register (caporal) {
    caporal
      ...
      .action((args, options) => new ExtensionAction().createExtension(args, options))
    ...
    caporal
      ...
      .action((args, options) => new ExtensionAction().attachExtensions(args, options))
    ...
    caporal
      ...
      .action((args, options) => new ExtensionAction().detachExtensions(args, options))

    caporal
      .command('extension manage')
      .description('Attaches and detaches extensions via a select picker')
      .action((args, options) => new ExtensionAction().manageExtensions(args, options))
  }
  ...
  createExtension ({types}, options) {
    ...
    const extensionsFolder = path.join(this.settings.getApplicationFolder(), EXTENSIONS_FOLDER)
  ...
  attachExtensions ({extensions = []}) {
    ...
    extensions.forEach((extensionFolderName) => {
      const extensionInfo = this._getExtensionInfo(extensionFolderName, true)
      if (!extensionInfo) {
      ...
      this.settings.attachExtension(extensionFolderName, {id: extensionInfo.id, trusted: extensionInfo.trusted}, force)

      utils.generateComponentsJson(this.settings)
    })
  }
  ...
  async manageExtensions () {
    ...
    // Get the extension summary and sort the entries alphabetically for better readability within the picker
    const extensionsSummary = this._getExtensionsSummary().sort((a, b) => a.id.localeCompare(b.id))

    if (extensionsSummary.length === 0) {
    ...
    const selected = (await inquirer.prompt({
      ...
      pageSize: 10,
      choices: extensionsSummary.map(({ id, attached }) => ({
        name: id,
        checked: attached
      }))
    })).extensions.map((selectedId) => {
      // Collect the dirnames of the selected extensions
      return extensionsSummary.find(({id}) => id === selectedId).dir
    })

    // Create dirname lists of the available extensions since they are needed later to attach in detach
    const attached = extensionsSummary.filter(({attached}) => attached).map(({ dir }) => dir)
    const detached = extensionsSummary.filter(({attached}) => !attached).map(({ dir }) => dir)
    ...
    if (toBeDetached.length) this.detachExtensions({extensions: toBeDetached})
    if (toBeAttached.length) this.attachExtensions({extensions: toBeAttached})
  }

  _getExtensionsSummary () {
    const attachedExtensions = Object.keys(this.settings.loadAttachedExtensions()).map((id) => {
      return id
    })

    return this._getAllExtensions().reduce((result, dir) => {
      const extensionInfo = this._getExtensionInfo(dir, true)
  ...
  _checkIfExtensionExists (extensionName) {
    const extensionsFolder = path.join(this.settings.getApplicationFolder(), EXTENSIONS_FOLDER, extensionName.replace('/', '-'))
  ...
  _getAllExtensions () {
    ...
    const exts = []
    files.forEach(file => {
      if (this._getExtensionInfo(file, true)) exts.push(file)
    })
    return exts
  }
   ...
   */
  _getExtensionInfo (extensionName, ignoreNotExisting) {
    const extensionsFolder = path.join(this.settings.getApplicationFolder(), EXTENSIONS_FOLDER)
