...
class ExtensionAction {
  ...
  static register (caporal, appSettings, userSettings, dcHttpClient) {
    ...
    caporal
      ...
      .description(t('VERSION_DESCRIPTION'))
      .argument('[extension]', t('VERSION_EXTENSION_DESCRIPTION'))
      .argument('[tag]', t('VERSION_TAG_DESCRIPTION'))
      .action(async (args) => {
        await ExtensionAction.build(appSettings, userSettings).versionExtension(args).catch(exceptionHandler)
      })
  ...
  _removePlaceholders (userInput, state) {
    ...
    return replace({
      ...
      .catch(err => {
        ...
        if (err.message.startsWith('No files match the pattern')) return logger.debug(t('ERROR_NO_FILES_MATCH_PATTERN', { pattern: filesPattern.join(', ') }))
      })
  }
  ...
  /**
   * Sets a new version in extension-config.json, commits it and creates the git tag v<version>.
   * Without a git repository only the config is updated and a warning is shown.
   * @param {Object} args
   * @param {String} [args.extension] Target extension folder name
   * @param {String} [args.tag] New version
   * @returns {Promise}
   */
  async versionExtension (args = {}) {
    await this.appSettings.validate()

    const extensionsFolder = path.join(this.appSettings.getApplicationFolder(), EXTENSIONS_FOLDER)
    let { extension: extensionDir, tag } = args

    // `extension version 1.2.3` without an extension: the single argument is the tag
    if (extensionDir && !tag && semver.valid(extensionDir) && !(await fsEx.exists(path.join(extensionsFolder, extensionDir)))) {
      tag = extensionDir
      extensionDir = undefined
    }

    const { extensionId, extensionDir: resolvedDir, extensionVersion } = await this._resolveExtensionInfo(extensionDir, false, t('INPUT_SELECT_EXTENSION_TO_VERSION'))

    const validateVersion = (input) => {
      const version = semver.valid(input)
      if (!version) return t('ERROR_INVALID_VERSION', { version: input })
      if (version === extensionVersion) return t('ERROR_VERSION_UNCHANGED', { version })
      // A current version that is not valid semver may still be replaced
      if (semver.valid(extensionVersion) && semver.lt(version, extensionVersion)) return t('ERROR_VERSION_LOWER', { version, extensionVersion })
      return true
    }
    ...
    if (!tag) {
      tag = (await inquirer.prompt({
        type: 'input',
        name: 'tag',
        message: t('INPUT_NEW_VERSION', { extensionId, extensionVersion }),
        validate: validateVersion
      })).tag
    }

    const validationResult = validateVersion(tag)
    if (validationResult !== true) throw new Error(validationResult)
    const version = semver.valid(tag)
    const gitTag = `v${version}`

    const extensionPath = path.resolve(extensionsFolder, resolvedDir)
    const isGitRepository = await this._git(extensionPath, ['rev-parse', '--is-inside-work-tree']).then(() => true, () => false)
    if (isGitRepository) {
      if (await this._git(extensionPath, ['status', '--porcelain'])) {
        throw new Error(t('ERROR_GIT_WORKING_TREE_NOT_CLEAN', { directory: resolvedDir }))
      }
      if (await this._git(extensionPath, ['tag', '-l', gitTag])) {
        throw new Error(t('ERROR_GIT_TAG_EXISTS', { tag: gitTag, directory: resolvedDir }))
      }
    }

    const extensionConfigPath = path.join(extensionPath, 'extension-config.json')
    const extensionConfig = await fsEx.readJson(extensionConfigPath)
    extensionConfig.version = version
    await fsEx.writeJson(extensionConfigPath, extensionConfig, { spaces: 2 })

    if (!isGitRepository) {
      logger.warn(t('WARN_NOT_A_GIT_REPOSITORY', { directory: resolvedDir }))
      logger.plain(t('VERSION_SET_SUCCESSFULLY', { extensionId, version }))
      return
    }

    await this._git(extensionPath, ['add', 'extension-config.json'])
    await this._git(extensionPath, ['commit', '-m', version])
    await this._git(extensionPath, ['tag', '-a', gitTag, '-m', version])

    logger.plain(t('VERSION_SET_AND_TAGGED_SUCCESSFULLY', { extensionId, version, tag: gitTag }))
  }

  /**
   * Runs a git command in the extension directory and resolves with its trimmed stdout.
   * @param {String} extensionPath
   * @param {String[]} args
   * @returns {Promise<String>}
   */
  _git (extensionPath, args) {
    return new Promise((resolve, reject) => {
      childProcess.execFile('git', args, { cwd: extensionPath }, (err, stdout, stderr) => {
        if (err) {
          return reject(new Error(t('ERROR_GIT_COMMAND_FAILED', { command: args.join(' '), message: (stderr || '').trim() || err.message })))
        }
        resolve(stdout.trim())
      })
    })
   ...
   * @param {String} [selectMessage] Prompt message when the user has to pick an extension
   * @returns {Promise}
   */
  async _resolveExtensionInfo (extensionDir, isTheme = false, selectMessage) {
    const extensionsDirectory = isTheme
      ? path.join(this.appSettings.getApplicationFolder(), THEMES_FOLDER)
      : path.join(this.appSettings.getApplicationFolder(), EXTENSIONS_FOLDER)
    ...
    if (extensionDir) {
      ...
      extensionDir = (await inquirer.prompt({
        type: 'list',
        name: 'extensionDir',
        message: selectMessage || (isTheme ? t('INPUT_SELECT_THEME_TO_UPLOAD') : t('INPUT_SELECT_EXTENSION_TO_UPLOAD')),
        pageSize: 10,
        choices
      })).extensionDir
    }
