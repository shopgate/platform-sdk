...
class ExtensionAction {
  ...
  static register (caporal, appSettings, userSettings, dcHttpClient) {
    ...
    caporal
      ...
      .description('Uploads local extensions to the DevCenter')
      .argument('[extension]', 'Folder name of the extension to upload')
  ...
  async uploadExtension (args = {}, options = {}) {
    ...
    if (args.extension) {
      ...
      if (!extension) {
        throw new Error(`Extension directory ${args.extension} does not exist`)
      }
      ...
      const selectedId = (await inquirer.prompt({
        type: 'list',
        name: 'extensionId',
        message: 'Select an extension to upload',
        pageSize: 10,
    ...
    logger.debug('Extension version:', extensionVersion)

    try {
      logger.debug(`Checking if extension ${extensionId} exists`)
    ...
    try {
      ...
      switch (res.status) {
        ...
        case 'UPLOADED':
          logger.debug('The extension is in UPLOADED status. Updating it to be DRAFT')
          await this.dcHttpClient.patchExtensionVersion(extensionId, extensionVersion, { ...res, status: 'DRAFT' })
          break

        case 'ERROR':
        case 'DRAFT':
          logger.debug(`The extension is in ${res.status} status. Can continue with upload`)
          break
      ...
      if (err instanceof NotFoundError) {
        ...
        if (!createVersion) {
          throw new Error(`Extension version ${extensionVersion} does not exist. Exiting.`)
        }
      } else {
        throw err
      }
    ...
    try {
      ...
      logger.debug('.gitignore does not exist in the extension directory. Using defaults')
    ...
    logger.info('Preprocessing extension (could take up to 2-3 minutes)...')
    const preprocessStartedAt = new Date()
    do {
      const version = await this.dcHttpClient.getExtensionVersion(extensionId, extensionVersion)

      logger.debug(`Status of extension version is ${version.status}`)
    ...
    logger.info('Extension version successfully uploaded')
  }
