...
const IS_THEME = Symbol('isTheme')

const { EXTENSIONS_FOLDER } = require('../app/Constants')

class ExtensionAction {
  static get IS_THEME () { return IS_THEME }
  ...
  static register (caporal, appSettings, userSettings, dcHttpClient) {
    caporal
      ...
      .action(async (args, options) => {
        try {
          await ExtensionAction.build(appSettings, userSettings).createExtension(args, options)
        } catch (err) {
    ...
    caporal
      ...
      .action(async (args) => {
        try {
          await ExtensionAction.build(appSettings, userSettings).attachExtensions(args)
        } catch (err) {
    ...
    caporal
      ...
      .action(async (args) => {
        try {
          await ExtensionAction.build(appSettings, userSettings).detachExtensions(args)
    ...
    caporal
      ...
      .action(async () => {
        try {
          await ExtensionAction.build(appSettings, userSettings).manageExtensions()
        } catch (err) /* istanbul ignore next */ {
          logger.error(err.message)
          process.exit(1)
        }
    ...
    caporal
      ...
      .description('Uploads local extension to the DevCenter')
      ...
      .option('--keep-file', 'Do not delete a temporary file after uploading')
      .option('--poll-interval', 'The interval of polling of the uploaded file status, milliseconds', caporal.INT, 3000)
      .action(async (args, options) => {
        try {
          await ExtensionAction.build(appSettings, userSettings, dcHttpClient).uploadExtension(args, options)
        } catch (err) /* istanbul ignore next */ {
          logger.error(err.message)
          process.exit(1)
        }
    ...
    caporal
      ...
      .description('Uploads local theme to the DevCenter')
      .argument('[extension]', 'Directory name of the theme to upload')
      .option('--force', 'Skip a prompt to create a new version and create the version automatically in draft')
      .option('--preprocess-timeout <ms>', 'Preprocessing timeout, milliseconds', caporal.INT, 12e4)
      .option('--keep-file', 'Do not delete a temporary file after uploading')
      .option('--poll-interval', 'The interval of polling of the uploaded file status, milliseconds', caporal.INT, 3000)
  ...
  _getUserInput (options, types = [], externalUserInput) {
    let trusted = false
    if (options.trusted === null) trusted = true
    else if (typeof options.trusted === 'boolean') trusted = options.trusted
  ...
  async manageExtensions () {
    ...
    if (extensionPropeties.length === 0) {
      return logger.warn('There are no extensions in folder \'./extensions/\'')
    }
   ...
   * @param {Boolean} [options.keepFile] Keep .tar.gz file in a temp directory after uploading
   * @returns {Promise}
   */
  async uploadExtension (args = {}, options = {}) {
    ...
    const isTheme = args[IS_THEME] === true
    ...
    let extensionDir
    let extensionVersionExists

    if (args.extension) {
      const extension = extensionPropeties.find(({ dir }) => dir === args.extension)

      if (!extension) {
        throw new Error(`${isTheme ? 'Theme' : 'Extension'} directory ${args.extension} does not exist`)
      }

      if (isTheme && extension.isTheme !== true) {
        logger.warn(`Extension ${extension.id} exists but is not marked as theme (type: "theme")`)
      }
      ...
      const selectedId = (await inquirer.prompt({
        type: 'list',
        name: 'extensionId',
        message: `Select an ${isTheme ? 'theme' : 'extension'} to upload`,
        pageSize: 10,
        choices: extensionPropeties.map(({ id }) => ({
          name: id
        }))
      })).extensionId

      const extension = extensionPropeties.find(({ id }) => id === selectedId)
      extensionId = extension.id
      extensionDir = extension.dir
    }
    ...
    let extensionConfig
    try {
      const extensionConfigStr = await fsEx.readFile(path.resolve(extensionPath, 'extension-config.json'))
      extensionConfig = JSON.parse(extensionConfigStr)
    } catch (err) {
      if (err.code === 'ENOENT') {
    ...
    if (!extensionConfig || !extensionConfig.version) {
      throw new Error('Invalid extension-config.json')
    }

    extensionVersion = extensionConfig.version
    logger.debug(`${isTheme ? 'Theme' : 'Extension'} version:`, extensionVersion)

    try {
      logger.debug(`Checking if ${isTheme ? 'theme' : 'extension'} ${extensionId} exists`)
      await this.dcHttpClient.getExtension(extensionId, extensionVersion)
    } catch (err) {
      const message = `Error getting ${extensionId}: ${err.message || 'Unknown error'}`
      throw new Error(message)
    }

    try {
      logger.debug(`Checking if version ${extensionVersion} exists`)
      const res = await this.dcHttpClient.getExtensionVersion(extensionId, extensionVersion)

      switch (res.status) {
        case 'PREPROCESSING':
          throw new Error('Another upload is in progress. Try later.')

        case 'UPLOADED':
          logger.debug(`The ${isTheme ? 'theme' : 'extension'} is in UPLOADED status. Updating it to be DRAFT`)
          await this.dcHttpClient.patchExtensionVersion(extensionId, extensionVersion, { ...res, status: 'DRAFT' })
          break

        case 'ERROR':
        case 'DRAFT':
          logger.debug(`The ${isTheme ? 'theme' : 'extension'} is in ${res.status} status. Can continue with upload`)
          break

        default:
          throw new Error(`Cannot not upload version when in status ${res.status}`)
      }

      extensionVersionExists = true
    } catch (err) {
      if (err instanceof NotFoundError) {
        const result = await inquirer.prompt({
          type: 'confirm',
          name: 'createVersion',
          message: `Version ${extensionVersion} does not exists. Create it automatically?`,
          default: true
        })
        const createVersion = options.force || result.createVersion

        if (!createVersion) {
          throw new Error(`${isTheme ? 'Theme' : 'Extension'} version ${extensionVersion} does not exist. Exiting.`)
        }
      } else {
        throw err
      }
    ...
    if (!extensionVersionExists) {
      logger.debug(`Creating version ${extensionVersion} for ${extensionDir}`)
      await this.dcHttpClient.createExtensionVersion(extensionId, { version: extensionVersion })
    }

    logger.info(`Packing ${isTheme ? 'theme' : 'extension'}...`)

    const tempDir = await fsEx.mkdtemp(`${os.tmpdir()}${path.sep}`)
    const tempFilePath = path.resolve(tempDir, `${extensionDir}.tar.gz`)
    logger.debug('Temporary file path:', tempFilePath)
    ...
    try {
      ...
      logger.debug(`.gitignore does not exist in the ${isTheme ? 'theme' : 'extension'} directory. Using defaults`)
    ...
    const tempFile = fsEx.createWriteStream(tempFilePath)
    ...
    logger.debug('Packing directory into tar archive...')
    ...
    logger.info(`Uploading ${isTheme ? 'theme' : 'extension'}...`)
    await this.dcHttpClient.uploadExtensionFile(extensionId, extensionVersion, tempFilePath)

    logger.info(`Preprocessing ${isTheme ? 'theme' : 'extension'} (could take up to 2-3 minutes)...`)
    const preprocessStartedAt = new Date()
    do {
      const version = await this.dcHttpClient.getExtensionVersion(extensionId, extensionVersion)

      logger.debug(`Status of ${isTheme ? 'theme' : 'extension'} version is ${version.status}`)
      switch (version.status) {
        case 'PREPROCESSING':
          if (Date.now() - preprocessStartedAt > options.preprocessTimeout) {
            throw new Error('Preprocessing timeout expired. Try increasing the timeout with --preprocess-timeout option')
          }
          logger.debug(`Still preprocessing. Will retry in ${options.pollInterval}ms`)
          await utils.sleep(options.pollInterval)
          continue
        ...
        case 'ERROR':
          logger.debug('Preprocessing failed. Fetching error log')
          const res = await this.dcHttpClient.getExtensionVersionLog(extensionId, extensionVersion)
          let message
          if (Array.isArray(res.logs) && res.logs.length) {
            message = res.logs.map(({ name }) => name).join('\n')
          } else {
    ...
    if (options.keepFile) {
      logger.info(`Uploadable left at ${tempFilePath}`)
    } else {
      logger.debug('Deleting temporary file')
      await fsEx.remove(tempDir)
    }

    logger.info(`${isTheme ? 'Theme' : 'Extension'} version successfully uploaded`)
  }
