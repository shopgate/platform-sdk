...
class ExtensionAction {
  ...
  static register (caporal, appSettings, userSettings, dcHttpClient) {
    ...
    caporal
      ...
      .option('-f, --force', 'Automatic yes to prompts. Assume "yes" as answer to all prompts')
      .option('-t, --preprocess-timeout <ms>', 'Preprocessing timeout, milliseconds', caporal.INT, 60e4)
      .option('-i, --poll-interval <ms>', 'The interval of polling of the uploaded file status, milliseconds', caporal.INT, 3000)
      .action(async (args, options) => {
  ...
  _renameBoilerplate (userInput, defaultPath, extensionsFolder, state) {
    ...
    return new Promise((resolve, reject) => {
      fsEx.rename(defaultPath, ePath, (err) => {
        ...
        state.moved = true
        logger.debug('Renaming boilerplate ... done')
        resolve()
      })
    })
  }
