...
class BackendAction {
  ...
  async run (cliOptions) {
    ...
    this.backendProcess.on('reconnect', async () => {
      ...
      await Promise.all([
        utils.writeLocalPipelines(this.dcHttpClient, appId, this.pipelinesFolder, this.trustedPipelinesFolder),
  ...
  async _startSubProcess () {
    ...
    await utils.writeLocalPipelines(this.dcHttpClient, applicationId, this.pipelinesFolder, this.trustedPipelinesFolder)
    ...
    this.extensionConfigWatcher.on('configChange', async (config) => {
      await extensionConfigChanged(config, this.appSettings, this.dcHttpClient, this.backendProcess)
      await this.backendProcess.reloadPipelineController()
    })
  ...
  async _pipelineRemoved (file, trusted) {
    ...
    try {
      ...
      await utils.writeLocalPipelines(this.dcHttpClient, appId, this.pipelinesFolder, this.trustedPipelinesFolder)
    } catch (err) {
  ...
  async _pipelineChanged (file, trusted) {
    ...
    logger.debug('Get updated pipelines')
    await utils.writeLocalPipelines(this.dcHttpClient, appId, this.pipelinesFolder, this.trustedPipelinesFolder)
