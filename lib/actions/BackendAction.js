...
class BackendAction {
  ...
  async _startSubProcess () {
    ...
    this.extensionConfigWatcher.on('configChange', (config) => this._configChangeHandler(config))
  ...
  async _configChangeHandler (config) {
    this._extensionChanged(config)

    // Create components.json if needed
    if (config.file.components && config.file.components.length > 0) {
      utils.generateComponentsJson(this.appSettings.getApplicationFolder(), config.file.id, config.file.components)
    }
  }
