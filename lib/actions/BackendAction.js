...
class BackendAction {
  ...
  static register (caporal) {
    caporal
      .command('backend start')
        .option('--inspect', 'enables inspoection of code')
      .description('Establish a connection to the development system')
      .action((args, options, logger) => new BackendAction().run('start', options))
  }
  ...
  run (action, options) {
    ...
    this.appSettings.inspect = options ? options.inspect : false
