...
class BackendAction {
  ...
  async _stop () {
    ...
    // Best effort: remove this session's hooks via HTTP so the cleanup does not depend
    // solely on the Developer Connector's socket-disconnect handler — its failure (e.g.
    // a connector restart during the session) is what leaves orphaned hooks behind.
    try {
      await this.dcHttpClient.clearHooks(await this.appSettings.getId())
    } catch (err) {
      logger.debug({ err }, 'Could not clear hooks during shutdown (best effort)')
    }
  ...
  async _startSubProcess () {
    ...
    // Clear leftover hooks of previous sessions via the Developer Connector's HTTP API
    // BEFORE connecting: orphaned hooks make the dev pipeline controller crash-loop on
    // boot (their step code only existed in the dead session), and selectApplication
    // below requires a running pipeline controller — clearing via the socket would come
    // too late. pushHooks below re-pushes the hooks of all attached extensions anyway.
    await this.dcHttpClient.clearHooks(applicationId)
