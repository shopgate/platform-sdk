...
class BackendAction {
  ...
  run (action) {
    ...
    this.extensionWatcher = new ExtensionConfigWatcher()
    ...
    process.on('SIGINT', () => {
      this.extensionWatcher.stop(() => {
        ...
        this.backendProcess.extensionWatcher.stop()
  ...
  _extensionChanged (config, cb = () => {}) {
    async.retry({
      ...
      this.dcClient.generateExtensionConfig(config.file, AppSettings.getInstance().getId(), (err, extConfig) => {
        if (err) return cb(new Error(`Could not generate Config for '${config.file.id}'`))
      ...
      if (!extConfig.backend) {
        logger.error('Backend-part is missing in generated config')
        return cb(new Error('Backend-part is missing in generated config'))
      }
      fsEx.outputFileSync(path.join('', config.path, 'extension', 'config.json'), JSON.stringify(extConfig.backend, null, 2), 'UTF-8')
      logger.info(`Updated extension ${config.file.id}`)
      cb(null, extConfig.backend)
    })
  }
}
