...
class BackendAction {
          ...
          this.pipelineWatcher.start(() => {
            this.pipelineWatcher.on('all', (event, file) => {
              fsEx.pathExists(file, (err, exists) => {
                ...
                if (!exists) return this._pipelineRemoved(file, (err) => logger.error(err))
                this._pipelineChanged(file, (err) => logger.error(err))
              })
            })
          })
  ...
  _pipelineRemoved (file, cb) {
    ...
    async.retry({
      ...
      if (err) {
        err = new Error(`Could not remove pipeline '${pipelineId}': ${err.message}`)
  ...
  _pipelineChanged (file, cb) {
    fsEx.readJson(file, (err, pipeline) => {
      ...
      async.retry({
        ...
        if (err) {
          err = new Error(`Could not upload pipeline '${pipeline.pipeline.id}': ${err.message}`)
          logger.error(err.message)
          return cb(err)
        }
