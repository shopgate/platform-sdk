...
class BackendAction {
  ...
  _extensionChanged (config, cb = () => {}) {
    this.dcClient.generateExtensionConfig(config.file, this.appSettings.getId(), async (err, extConfig) => {
      if (err) throw new Error(`Could not generate config for '${config.file.id}': ${err.code}`)
