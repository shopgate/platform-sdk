...
class BackendAction {
  ...
  async writeExtensionConfigs () {
    ...
    for (let extensionId in attachedExtensions) {
      const config = attachedExtensions[extensionId]
      promises.push(async () => this._extensionConfigChanged(config, this.appSettings.getId()))
    }
  ...
  async _startSubProcess () {
    ...
    this.extensionConfigWatcher.on('configChange', (config) => this._extensionConfigChanged(config))
  ...
  // todo: rename
  async _extensionConfigChanged (config) {
    try {
