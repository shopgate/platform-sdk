...
class BackendAction {
  ...
  _startSubProcess () {
    this.backendProcess.connect(err => {
      ...
      this.dcClient.downloadPipelines(this.appSettings.getId(), (err, pipelines) => {
        if (err) throw err
  ...
  _pipelineRemoved (file, cb) {
    ...
    async.retry({
      ...
      this.dcClient.removePipeline(pipelineId, this.appSettings.getId(), (err) => {
  ...
  _pipelineChanged (file, cb) {
    fsEx.readJson(file, {throws: false}, (err, pipeline) => {
      ...
      async.retry({
        ...
        this.dcClient.uploadPipeline(pipeline, this.appSettings.getId(), (err) => {
          if (err) logger.debug(err)
          acb(err)
        })
      }, (err) => {
        if (err) return cb(new Error(`Could not upload pipeline '${pipeline.pipeline.id}': ${err.message}`))
