...
class BackendAction {
  ...
  _writeLocalPipelines (cb) {
    async.parallel([
      cb => this.dcClient.downloadPipelines(AppSettings.getInstance().getId(), false, cb),
      cb => this.dcClient.downloadPipelines(AppSettings.getInstance().getId(), true, cb)
    ], (err, pipelines) => {
      if (err) throw err
      async.parallel([
        cb => this._writePipelines(pipelines[0], this.pipelinesFolder, cb),
        cb => this._writePipelines(pipelines[1], this.trustedPipelinesFolder, cb)
      ], cb)
    })
  }
