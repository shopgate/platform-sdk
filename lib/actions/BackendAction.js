...
class BackendAction {
    ...
    process.on('SIGINT', () => {
      this.pipelineWatcher.stop(() => {
        this.extensionConfigWatcher.stop(() => {
          this.backendProcess.extensionWatcher.close((err) => {
            if (err) this.log.error(err)
            this.backendProcess.disconnect((err2) => {
              if (err2) this.log.error(err2)
  ...
  _extensionChanged (config, cb = () => {}) {
    async.retry({
      ...
      if (err) return logger.error('Could not generate a new extension config')
