...
class BackendAction {
      ...
      this.dcClient.getPipelines(AppSettings.getInstance().getId(), (err, pipelines) => {
        ...
        this._updatePipelines(pipelines, (err) => {
          ...
          this.pipelineWatcher.start(() => {
            this.pipelineWatcher.on('pipelineChanged', (pipeline) => this._pipelineChanged(pipeline))
          })
  ...
  _updatePipelines (pipelines, cb) {
    ...
    fsEx.emptyDir(pipelineFolder, (err) => {
      ...
      async.each(pipelines, (pipeline, eCb) => fsEx.writeJson(path.join(pipelineFolder, `${pipeline.pipeline.id}.json`), pipeline, eCb), cb)
    })
  }
