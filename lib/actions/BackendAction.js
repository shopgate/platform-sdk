...
class BackendAction {
  ...
  static register (caporal, appSettings) {
    caporal
      ...
      .action(() => new BackendAction(appSettings).run())
  }
  ...
  constructor (appSettings) {
    ...
    this.pipelinesFolder = path.join(this.appSettings.getApplicationFolder(), AppSettings.PIPELINES_FOLDER)
    this.trustedPipelinesFolder = path.join(this.appSettings.getApplicationFolder(), AppSettings.TRUSTED_PIPELINES_FOLDER)
  }

  async run () {
    this.userSettings = await new UserSettings().validate()
    ...
    this.backendProcess = new BackendProcess(this.userSettings, this.appSettings, logger)
  ...
  async _pipelineEvent (event, file) {
    const extensionFolder = path.join(this.appSettings.getApplicationFolder(), AppSettings.EXTENSIONS_FOLDER, path.sep)
  ...
  async _pipelineRemoved (file, trusted) {
    ...
    try {
      await this.dcClient.removePipeline(pipelineId, await this.appSettings.getId(), trusted)
  ...
  async _writeLocalPipelines () {
    ...
    await Promise.all([
      (async () => {
        const pipelines = await this.dcClient.downloadPipelines(appId, false)
        await this._writePipelines(pipelines, this.pipelinesFolder)
      })(),
      (async () => {
        // download trusted pipelines
        const pipelines = await this.dcClient.downloadPipelines(appId, true)
  ...
  async _pipelineChanged (file, trusted) {
    ...
    await this.dcClient.uploadPipeline(pipeline, await this.appSettings.getId(), trusted)

    // update pipeline in local pipelines folder
    logger.debug('Update pipeline file in pipelines folder')
    const pipelinePath = path.join(this.appSettings.getApplicationFolder(), (trusted) ? AppSettings.TRUSTED_PIPELINES_FOLDER : AppSettings.PIPELINES_FOLDER, `${pipeline.pipeline.id}.json`)
  ...
  async _extensionChanged (config) {
    try {
      const extConfig = await this.dcClient.generateExtensionConfig(config.file, await this.appSettings.getId())
  ...
  async _uploadPipelinesOfAttachedExtenstions () {
    ...
    await Promise.all([
      this.dcClient.uploadMultiplePipelines(pipelines, appId, false),
      this.dcClient.uploadMultiplePipelines(trustedPipelines, appId, true)
    ])
