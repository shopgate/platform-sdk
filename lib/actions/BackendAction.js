...
class BackendAction {
  ...
  _pipelineRemoved (file, cb) {
    ...
    async.retry({
      ...
      this.backendProcess.triggerPipelineReload((err) => {
        if (err) return cb(new Error(`Could not remove pipeline '${pipelineId}': ${err.message}`))
  ...
  _pipelineChanged (file, cb) {
    async.waterfall([
      (acb) => {
        ...
        logger.debug(`Pipeline changed -- Read file ${file}`)
        fsEx.readJson(file, {throws: false}, (err, pipeline) => {
          if (err) return acb(err)
          if (!pipeline || !pipeline.pipeline || !pipeline.pipeline.id) return acb(new Error('invalid pipeline'))
      ...
      (pipeline, acb) => {
        ...
        const isTrusted = file.includes(AppSettings.TRUSTED_PIPELINES_FOLDER)
        this._uploadPipeline(pipeline, isTrusted, acb)
      },
      (acb) => {
        // reload (t)plc
        logger.debug(`Pipeline changed -- Trigger reload`)
        this.backendProcess.triggerPipelineReload((err) => {
          acb(err)
        })
      }
  ...
  _uploadPipeline (pipeline, isTrusted, cb) {
    async.retry({
      times: 5,
      interval: (count) => 100 * Math.pow(2, count),
      errorFilter: (err) => { return err.statusCode !== 400 }
    }, (acb) => {
      this.dcClient.uploadPipeline(pipeline, AppSettings.getInstance().getId(), isTrusted, (err) => {
        acb(err)
      })
    },
      (err) => cb(err)
  ...
  _extensionConfigChanged (config, cb = () => {}) {
    async.retry({
      times: 5,
