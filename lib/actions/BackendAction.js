...
class BackendAction {
  ...
  async _pipelineEvent (event, file) {
    const extensionFolder = path.join(this.appSettings.getApplicationFolder(), AppSettings.EXTENSIONS_FOLDER, '/')
    ...
    const pathOfChangedExtension = file.replace(extensionFolder, '').split('/')[0]
  ...
  async _pipelineChanged (file, trusted) {
    ...
    await fsEx.writeJSON(pipelinePath, pipeline)
