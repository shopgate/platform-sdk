...
class BackendAction {
  ...
  async _writeLocalPipelines () {
    ...
    await Promise.all([
      this.getAndSavePipelines(appId, false, false, this.pipelinesFolder),
      this.getAndSavePipelines(appId, true, false, this.trustedPipelinesFolder)
    ])
    await Promise.all([
      this.getAndSavePipelines(appId, false, true, path.join(this.pipelinesFolder, 'resolved')),
      this.getAndSavePipelines(appId, true, true, path.join(this.trustedPipelinesFolder, 'resolved'))
    ])
  }
   ...
   */
  async getAndSavePipelines (appId, trusted, resolved, destination) {
    const pipelines = await this.dcHttpClient.downloadPipelines(appId, trusted, resolved)
    return this._writePipelines(pipelines, destination)
  }
  ...
  async _uploadPipelinesOfAttachedExtenstions () {
    ...
    for (let extensionId in attachedExtensions) {
      ...
      try {
        ...
        logger.debug(`Skipping pipeline upload of ${extensionId}: no pipelines folder found`)
        continue
      }
      ...
      if (pipelinesFiles < 1) {
        logger.debug(`Skipping pipeline upload of ${extensionId}: no pipelines in pipeline folder`)
        continue
      }
