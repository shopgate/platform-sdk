...
class BackendAction {
  ...
  async run (cliOptions) {
    ...
    await utils.ensureProjectGitignore(this.appSettings.getApplicationFolder())
    await utils.removeGeneratedBackendConfigs(this.appSettings.getApplicationFolder())
    await utils.removeLegacyBackendConfigs(this.appSettings.getApplicationFolder())
    ...
    const stop = async () => {
      ...
      try {
        await Promise.all([
          utils.removeGeneratedBackendConfigs(this.appSettings.getApplicationFolder()),
          utils.removeLegacyBackendConfigs(this.appSettings.getApplicationFolder())
        ])
      } catch (err) {
        logger.error(err)
        exitCode = 1
  ...
  async writeExtensionConfigs () {
    return utils.writeExtensionConfigs(this.appSettings, this.dcHttpClient, 'backend')
  }
  ...
  async _startSubProcess () {
    ...
    this.extensionConfigWatcher.on('configChange', async (config) => {
      const valid = await extensionConfigChanged(config, this.appSettings, this.dcHttpClient, 'backend')
      try {
