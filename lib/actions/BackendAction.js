...
class BackendAction {
  ...
  static register (commander) {
    commander
      ...
      .option('-i, --inspect', 'starts the debugger')
      .action((action) => new BackendAction().run(action, commander.args[1]))
  }
  ...
  run (action, command) {
    ...
    this.appSettings.inspect = command ? command.inspect : false
  ...
  _pipelineRemoved (file, cb) {
    ...
    async.retry({
      ...
      this.dcClient.removePipeline(pipelineId, this.appSettings.getId(), file.includes(AppSettings.TRUSTED_PIPELINES_FOLDER), (err) => {
  ...
  _writeLocalPipelines (cb) {
    async.parallel([
      tcb => this.dcClient.downloadPipelines(this.appSettings.getId(), false, (err, pipelines) => {
        if (err) tcb(err)
        this._writePipelines(pipelines, this.pipelinesFolder, tcb)
      }),
      tcb => this.dcClient.downloadPipelines(this.appSettings.getId(), true, (err, pipelines) => {
        if (err) tcb(err)
  ...
  _pipelineChanged (file, cb) {
    fsEx.readFile(file, 'utf8', (err, pipelineFileContent) => {
      ...
      async.whilst(
        ...
        (wcb) => {
          setTimeout(() => {
            this.dcClient.uploadPipeline(pipeline, this.appSettings.getId(), file.includes(AppSettings.TRUSTED_PIPELINES_FOLDER), (err) => {
              lastError = err
              if (err) {
                logger.debug(err)
  ...
  _extensionChanged (config, cb = () => {}) {
    async.retry({
      ...
      this.dcClient.generateExtensionConfig(config.file, this.appSettings.getId(), (err, extConfig) => {
