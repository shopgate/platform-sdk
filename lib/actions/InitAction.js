...
class InitAction {
    ...
    caporal
      ...
      .action(async (args, options) => {
        try {
          ...
          return logger.info(`The Application "${appId}" was successfully initialized`)
        } catch (err) {
  ...
  async checkForRunningProcesses (prompt) {
    const processes = []
    if (await utils.previousProcess('backend', SETTINGS_FOLDER)) processes.push('backend')
    if (await utils.previousProcess('frontend', SETTINGS_FOLDER)) processes.push('frontend')
  ...
  async initApplication (appId, userSettings, reset) {
    try {
      ...
      logger.debug(err)
      if (err instanceof UnauthorizedError) throw new Error('You\'re not logged in! Please run `sgcloud login` again.')
