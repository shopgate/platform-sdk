    ...
    try {
      ...
      return this.getAppId(inquirer.prompt, (err, appId) => {
        ...
        new DcHttpClient(userSettings).getApplicationData(appId, (err) => {
          if (err) {
            logger.debug(err)
            err = new Error(`The application ${appId} is not available or permissions are missing (message: ${err.message}). Please check the application at developer.shopgate.com!`)
            if (cb) return cb(err)
            else throw err
          }
