...
class InitAction {
  ...
  register (commander) {
    commander
      ...
      .option('--application_id <appId>', 'set the Sandbox App ID you want to initialize')
   ...
   */
  getAppId (prompt, cb) {
    if (this.options['application_id']) return cb(null, this.options['application_id'])
    prompt([{type: 'input', name: 'application_id', message: 'Enter your Sandbox App ID:'}]).then(answers => cb(null, answers['application_id']))
  }
}
