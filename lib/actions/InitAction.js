...
class InitAction {
  ...
  run (options, cb) {
    ...
    if (!token) return cb(new Error('You\'re not logged in! Please run `sgcloud login` again.'))
        ...
        new DcHttpClient(userSettings).getApplicationData(appId, (err) => {
          if (err) {
            ...
            if (cb) {
              ...
              return cb(new Error(`The application ${appId} is not available or permissions are missing (message: ${err.message}). Please check the application at developer.shopgate.com!`))
            }
          }
