...
class InitAction {
  ...
  /**
   * If cb returns true, init should stop
   */
  checkForRunningProcesses (prompt, cb) {
    const processes = []
    if (utils.isProcessRunning('backend', AppSettings.SETTINGS_FOLDER)) processes.push('backend')
    if (utils.isProcessRunning('frontend', AppSettings.SETTINGS_FOLDER)) processes.push('frontend')
