...
const THEME_CONFIG_READY = require('@shopgate/cloud-sdk-webpack/event').THEME_CONFIG_READY
const onThemeIsSet = require('@shopgate/cloud-sdk-webpack/event').onThemeIsSet
const event = require('@shopgate/cloud-sdk-webpack/event')
...
class FrontendAction {
  ...
  run (action, args, options = {}) {
    ...
    switch (action) {
      default:
      case 'start': {
        onThemeIsSet((templateFolder) => {
          this.updateThemeConfig(templateFolder)
            .then(() => event.emit(THEME_CONFIG_READY))
            .then(() => {
