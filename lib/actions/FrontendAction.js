...
class FrontendAction {
  ...
  async run (action, args, options = {}) {
    ...
    switch (action) {
      ...
      case 'start': {
        ...
        await utils.writeExtensionConfigs(this.appSettings, this.dcHttpClient, 'frontend')
  ...
  async start (options, themeFolder) {
    ...
    this.extensionConfigWatcher.on('configChange', (config) => extensionConfigChanged(config, this.appSettings, this.dcHttpClient, 'frontend'))
