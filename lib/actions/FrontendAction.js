...
class FrontendAction {
  ...
  async findThemes () {
    ...
    const result = await fsEx.readdir(source)
    const folders = result.filter(folder => fs
      .lstatSync(path.join(source, folder))
      .isDirectory()
    )
  ...
  run (action, args, options = {}) {
    ...
    switch (action) {
      ...
      case 'start': {
        if (options.theme) {
          return this.start(options, this.buildThemePath(options.theme))
        }

        this.requestThemeOption()
          .then((theme) => this.start({ ...options, theme }, this.buildThemePath(theme)))
          .catch((error) => {
            console.error(error)
            process.exit(1)
          })
