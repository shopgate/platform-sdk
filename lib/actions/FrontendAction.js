...
class FrontendAction {
  ...
  async run (action, args, options = {}) {
    ...
    switch (action) {
      default:
      case 'start': {
        if (options.theme) return this.start(options, await this.buildThemePath(options.theme))

        return this.requestThemeOption()
          .then(async (theme) => this.start({ ...options, theme }, await this.buildThemePath(theme)))
      }
      case 'setup': {
