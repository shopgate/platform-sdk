...
class FrontendAction {
  ...
  run (action, options = {}) {
    if (!UserSettings.getInstance().getSession().getToken()) {
      throw new Error('You\'re not logged in! Please run `sgcloud login` again.')
    }

    const pid = utils.previousProcess('frontend', this.settingsFolder)
    if (pid) throw new Error(`Frontend process is already running with pid: ${pid}. Please cancel this process first.`)
